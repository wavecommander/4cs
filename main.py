...
EXPORT_PATH = Path(Path.home() / f"Pictures/.4c/")
...
async def scrape_thread(thread_url, export_path, session):
    ...
    async with session.get(thread_url) as r:
        if r.status == 200:
            ...
            print(f"{r.status} status code from GET request: {r.url}")

    thread_folder_name = f"{board_id}-{thread_id}"
    thread_title = soup.find("span", {"class": "subject"}).string
    if thread_title:
        thread_title = thread_title.replace(" ", "-").replace("/", "_slash_").lower()
        thread_folder_name += "-" + thread_title
    thread_folder_path = export_path / thread_folder_name
    Path(thread_folder_path).mkdir(parents=True, exist_ok=True)

    img_urls = [
        thumb.attrs["href"].replace("//", "https://")
        for thumb in soup.find_all("a", {"class": "fileThumb"})
    ]
...
async def main():
    ...
    if len(sys.argv) < 2:
        thread_urls = [
            url for url in input("Thread URL(s): ").split() if url.startswith("http")
        ]
    else:
    ...
    async with aiohttp.ClientSession() as session:
        thread_coroutines = [
            scrape_thread(url, EXPORT_PATH, session) for url in thread_urls
        ]
